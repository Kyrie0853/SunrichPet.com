...
export default function PostDetail({post}:{post:CommunityPost}){
  ...
  return (<article>
    <div className="flex items-center gap-3">
      <Avatar userId={post.author_id} avatarUrl={post.author?.avatar_url} displayName={post.author?.display_name} size={48} clickable />
    ...
    {post.images?.length>0&&(<div className="mt-6 grid grid-cols-2 gap-2">{post.images.map((url,i)=>(<img key={i} src={url} className="rounded-xl object-cover w-full" alt="" loading="lazy" />))}</div>)}
