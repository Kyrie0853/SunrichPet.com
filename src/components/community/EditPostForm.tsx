...
import { CATEGORY_LABELS, CommunityCategory, CommunityPost } from "@/lib/supabase/community-types";
...
export default function EditPostForm({ post }: { post: CommunityPost }) {
  ...
  const [category, setCategory] = useState<CommunityCategory>(post.category as CommunityCategory);
  ...
  useEffect(() => { supabase.from("community_tags").select("id,name,slug").order("name").then(({ data }) => { if (data) setTags(data); }); }, [supabase]);
  ...
  return (<form onSubmit={handleSubmit} className="space-y-6">
    ...
    <div><label className="mb-2 block text-sm font-semibold text-gray-700">分类</label><select value={category} onChange={e=>setCategory(e.target.value as CommunityCategory)} className="w-full rounded-xl border border-gray-200 px-4 py-3 text-gray-900 outline-none focus:border-emerald-500">{(Object.entries(CATEGORY_LABELS) as [CommunityCategory,string][]).map(([k,l])=>(<option key={k} value={k}>{l}</option>))}</select></div>
