...
export default function CommentSection({postId,initialComments,postAuthorId}:{postId:string;initialComments:any[];postAuthorId:string}){
  ...
  useEffect(()=>{supabase.auth.getUser().then(({data:{user}})=>setCurrentUser(user));},[supabase]);

  const submitComment=async()=>{
    if(!currentUser||!newComment.trim())return;
    setSubmitting(true);
    const{data,error}=await supabase.from("community_comments").insert({post_id:postId,author_id:currentUser.id,content:newComment.trim()}).select("*,author:profiles!community_comments_author_id_fkey(id,display_name,avatar_url)").single();
    if(!error&&data){setComments(prev=>[...prev,{...data,replies:[]}]);setNewComment("");}
    setSubmitting(false);
  };

  const submitReply=async(parentId:string)=>{
    if(!currentUser||!replyContent.trim())return;
    const{data,error}=await supabase.from("community_comments").insert({post_id:postId,author_id:currentUser.id,parent_id:parentId,content:replyContent.trim()}).select("*,author:profiles!community_comments_author_id_fkey(id,display_name,avatar_url)").single();
    if(!error&&data){setComments(prev=>prev.map(c=>c.id===parentId?{...c,replies:[...(c.replies||[]),data]}:c));setReplyContent("");setReplyTo(null);}
  };

  return (<div><div className="mb-6">{currentUser?(<div><textarea value={newComment} onChange={e=>setNewComment(e.target.value)} placeholder="写下你的评论..." rows={3} className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm outline-none focus:border-emerald-500" /><div className="mt-2 flex justify-end"><button onClick={submitComment} disabled={submitting||!newComment.trim()} className="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:opacity-50">{submitting?"提交中...":"发表评论"}</button></div></div>):(<div className="rounded-xl bg-gray-50 px-4 py-3 text-center text-sm text-gray-400">请<a href="/auth" className="text-emerald-600 underline">登录</a>后发表评论</div>)}</div><div className="space-y-4">{comments.map(comment=>(<div key={comment.id} className="rounded-xl border border-gray-100 bg-white p-4"><div className="flex items-center gap-2"><div className="h-8 w-8 rounded-full bg-emerald-100 flex items-center justify-center text-xs font-bold text-emerald-600">{comment.author?.display_name?.charAt(0)||"U"}</div><span className="text-sm font-semibold text-gray-900">{comment.author?.display_name||"匿名"}{comment.author_id===postAuthorId&&(<span className="ml-1 rounded bg-emerald-100 px-1.5 py-0.5 text-xs text-emerald-700">作者</span>)}</span><span className="text-xs text-gray-400">{timeAgo(comment.created_at)}</span></div><p className="mt-2 text-sm text-gray-700 whitespace-pre-wrap">{comment.content}</p><div className="mt-2">{currentUser&&(<button onClick={()=>setReplyTo(replyTo===comment.id?null:comment.id)} className="text-xs text-gray-400 hover:text-emerald-600">回复</button>)}{replyTo===comment.id&&(<div className="mt-2"><textarea value={replyContent} onChange={e=>setReplyContent(e.target.value)} placeholder="写下回复..." rows={2} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none focus:border-emerald-500" /><div className="mt-2 flex gap-2 justify-end"><button onClick={()=>{setReplyTo(null);setReplyContent("")}} className="rounded-lg px-3 py-1 text-xs text-gray-500">取消</button><button onClick={()=>submitReply(comment.id)} disabled={!replyContent.trim()} className="rounded-lg bg-emerald-600 px-3 py-1 text-xs font-semibold text-white disabled:opacity-50">回复</button></div></div>)}</div>{comment.replies?.length>0&&(<div className="ml-8 mt-2 space-y-2 border-l-2 border-gray-100 pl-4">{comment.replies.map((reply:any)=>(<div key={reply.id}><div className="flex items-center gap-2"><div className="h-6 w-6 rounded-full bg-gray-100 flex items-center justify-center text-xs font-bold text-gray-500">{reply.author?.display_name?.charAt(0)||"U"}</div><span className="text-sm font-semibold text-gray-900">{reply.author?.display_name||"匿名"}{reply.author_id===postAuthorId&&(<span className="ml-1 rounded bg-emerald-100 px-1 py-0.5 text-xs text-emerald-700">作者</span>)}</span><span className="text-xs text-gray-400">{timeAgo(reply.created_at)}</span></div><p className="mt-1 text-sm text-gray-700">{reply.content}</p></div>))}</div>)}</div>))}</div>{comments.length===0&&(<p className="py-8 text-center text-sm text-gray-400">暂无评论，来发表第一条评论吧</p>)}</div>);
}