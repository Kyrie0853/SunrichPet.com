...
export default function Avatar({ userId, avatarUrl, displayName, size = 40, editable = false, onAvatarChange }: Props) {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_SIZE) { alert("图片大小不能超过 2MB"); return; }
    setUploading(true);
    try {
      const ext = file.name.split(".").pop() || "jpg";
      const fileName = userId + "/avatar-" + Date.now() + "." + ext;
      // 删除旧头像
      if (url) { const oldPath = url.split("/").slice(-2).join("/"); supabase.storage.from("community-images").remove([oldPath]).then(()=>{},()=>{}); }
      const { error: upErr } = await supabase.storage.from("community-images").upload(fileName, file, { contentType: file.type, upsert: true });
      ...
      if (updateErr) { alert("保存失败: " + updateErr.message); setUploading(false); return; }
      setUrl(newUrl);
      if (onAvatarChange) onAvatarChange(newUrl);
    } catch (err: any) { alert("上传异常: " + (err.message || "")); }
    setUploading(false);
    e.target.value = "";
  };
