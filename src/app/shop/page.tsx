...
import { getProductsByStatus, getAllSpecies, getAllMorphs, getTopCategories, type StudioProduct, type CategoryWithCount } from "@/lib/studio/products";
...
type Props = { searchParams: Promise<{ status?: string; species?: string; morph?: string; min_price?: string; max_price?: string; category?: string }> };
...
export default async function ShopPage({ searchParams }: Props) {
  ...
  const { status, species, morph, min_price, max_price, category } = await searchParams;

  const products = await getProductsByStatus(status, species, morph, min_price, max_price, category);
  ...
  function buildHref(overrides: Record<string, string | undefined>) {
    ...
    const sp = overrides.species !== undefined ? overrides.species : species;
    const m = overrides.morph !== undefined ? overrides.morph : morph;
    ...
    if (sp && sp !== "all") params.set("species", sp);
    if (m && m !== "all") params.set("morph", m);
  ...
  // 找到当前分类名称
  const currentCategory = topCategories.find(c => c.slug === category);
    ...
    <div className="mx-auto max-w-5xl px-4 py-6 md:py-10">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <h1 className="text-2xl md:text-3xl font-bold text-[#1f2937]">
            {currentCategory ? currentCategory.name : "全部个体"}
          </h1>
          <p className="text-[14px] text-[#6b7280] mt-0.5">支付宝担保交易，安心选购</p>
        </div>
        {isAdmin && (
          <Link
      ...
      {topCategories.length > 0 && (
        <div className="mb-3 flex flex-wrap gap-2">
          <Link href={buildHref({ category: "all" })}
            className={"rounded-full px-3 py-1.5 text-[12px] font-medium transition-all " +
              (!category ? "bg-[#e8f5ef] text-[#1a7f5a]" : "border border-[#e5e7eb] text-[#6b7280] hover:border-[#1a7f5a]")}>
            全部分类
          </Link>
          {topCategories.map((cat) => (
            <Link key={cat.slug} href={buildHref({ category: cat.slug })}
              className={"rounded-full px-3 py-1.5 text-[12px] font-medium transition-all " +
                (category === cat.slug ? "bg-[#e8f5ef] text-[#1a7f5a]" : "border border-[#e5e7eb] text-[#6b7280] hover:border-[#1a7f5a]")}>
              {cat.name}
            </Link>
          ))}
        </div>
      ...
      {allSpecies.length > 0 && (
        <div className="mb-3 flex flex-wrap gap-2">
          <Link href={buildHref({ species: "all" })}
            className={"rounded-full px-3 py-1.5 text-[12px] font-medium transition-all " +
          ...
          {allSpecies.map((sp) => {
            ...
            return (
              <Link key={sp} href={buildHref({ species: sp })}
                className={"rounded-full px-3 py-1.5 text-[12px] font-medium transition-all " +
      ...
      {allMorphs.length > 0 && (
        <div className="mb-3 flex flex-wrap gap-2">
          <Link href={buildHref({ morph: "all" })}
            className={"rounded-full px-3 py-1.5 text-[12px] font-medium transition-all " +
              (!morph ? "bg-[#e8f5ef] text-[#1a7f5a]" : "border border-[#e5e7eb] text-[#6b7280] hover:border-[#1a7f5a]")}>
            全部品系
          </Link>
          {allMorphs.map((m) => {
            const isActive = m === morph;
            return (
              <Link key={m} href={buildHref({ morph: m })}
                className={"rounded-full px-3 py-1.5 text-[12px] font-medium transition-all " +
                  (isActive ? "bg-[#e8f5ef] text-[#1a7f5a]" : "border border-[#e5e7eb] text-[#6b7280] hover:border-[#1a7f5a]")}>
                {m}
              </Link>
              ...
              <div key={product.id} className="relative group/card">
                <Link href={`/shop/product/${product.id}`}
                  className={"block bg-white rounded-xl border overflow-hidden transition-all " +
                    (isSold ? "border-[#f3f4f6] opacity-60 hover:opacity-80" : "border-[#f3f4f6] hover:shadow-md hover:border-[#1a7f5a]/20")}>
                  <div className="aspect-square bg-gray-100 overflow-hidden relative">
                    {product.images && product.images.length > 0 ? (
                      <img src={product.images[0]} alt={product.name}
                        className={"w-full h-full object-cover transition-transform duration-300 " + (isSold ? "grayscale" : "group-hover/card:scale-105")}
                        loading="lazy" />
                    ) : (
                  ...
                  <div className="p-3">
                    ...
                    <h3 className={"text-[14px] font-semibold line-clamp-1 transition-colors " + (isSold ? "text-[#9ca3af]" : "text-[#1f2937] group-hover/card:text-[#1a7f5a]")}>
                      {product.name}
                    </h3>
                    {product.morph && <p className="text-[12px] text-[#6b7280] mt-0.5 line-clamp-1">{product.morph}</p>}
                    <p className={"mt-2 text-[17px] font-bold " + (isSold ? "text-[#9ca3af]" : "text-[#1a7f5a]")}>¥{product.price}</p>
                  </div>
                </Link>
                {isAdmin && (
                  <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover/card:opacity-100 transition-opacity z-10">
                    <Link href={`/studio/dashboard/products/${product.id}/edit`}
                      className="rounded-lg bg-white/90 backdrop-blur-sm border border-[#e5e7eb] p-1.5 text-[#6b7280] hover:text-[#1a7f5a] shadow-sm transition-colors" title="编辑">
                    ...
                    <form>
                      <button type="submit"
                        ...
                        className="rounded-lg bg-white/90 backdrop-blur-sm border border-[#e5e7eb] p-1.5 text-[#6b7280] hover:text-[#dc3545] shadow-sm transition-colors" title="删除"
                        onClick={(e) => { if (!confirm("确定要删除？")) e.preventDefault(); }}>
                        <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
